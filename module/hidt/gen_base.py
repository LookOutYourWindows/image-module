...
from .content_encoders import ContentEncoderUnet
from .style_encoders import StyleEncoder
from .decoders import DecoderUnet
from .utils_base import weights_init, get_total_data_dim, split_tensor_to_maps
...
class GeneratorBase(nn.Module):
    ...
    def _init_modules(self, params):
        for module_name, module_config in params.items():
            ...
            if architecture == 'ContentEncoderUnet':
                setattr(self, module_name, ContentEncoderUnet(**module_config_copy))

            elif architecture == 'StyleEncoder':
                setattr(self, module_name, StyleEncoder(**module_config_copy))
                
            elif architecture == 'DecoderUnet':
                setattr(self, module_name, DecoderUnet(**module_config_copy))
