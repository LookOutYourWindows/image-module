...
from .gen_content_style_unet import GeneratorContentStyleUnet
...
class TrainerBase(nn.Module):
    ...
    def _init_models(self):
        ...
        for model_name, model_config in self.params['models'].items(): # self.params['models'] = ('gen': dictionary of settings)
            ...
            # For CUDA
            # setattr(self, model_name, GeneratorContentStyleUnet(model_config).cuda())
            
            # FOR CPU
            setattr(self, model_name, GeneratorContentStyleUnet(model_config).cpu())
